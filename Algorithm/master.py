...
class RL_master(object):
    def __init__(self, nb_states, nb_actions, variant, device, seed):
        ...
        # Hyper-parameters
        self.batch_size = variant['mbsize']
        self.batch_size_ft = variant['bsize']
    ...
    def update_policy(self, finetune=False):                    
    ...
    def grad_calculation(self):    
        ...
        for item in self.critic.parameters():
            q_grad.append(item.grad)
        ...
        for item in self.guard.parameters():
            g_grad.append(item.grad)
        ...
        for item in self.actor.parameters():
            actor_grad.append(item.grad)

        # Actor grad w.r.t. q values                                               
        self.actor.zero_grad()
        ...
        for item in self.actor.parameters():
            aq_grad.append(item.grad)

        # Actor grad w.r.t. g values 
        self.actor.zero_grad()

        policy_loss = - (self.guard([to_tensor(state_batch),self.actor(to_tensor(state_batch))]))

        policy_loss = policy_loss.mean()
        policy_loss.backward()

        for item in self.actor.parameters():
            ag_grad.append(item.grad)

        return q_grad, g_grad, actor_grad, aq_grad, ag_grad
