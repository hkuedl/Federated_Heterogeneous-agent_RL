...
def _compute_layer_weights(
    ...
    for key in updates[0].keys():
        ...
        l2_scores = 1 / (1 + torch.abs((norms - norm_median) / norm_std))
        ...
        I_sign = torch.ones_like(sign_sum) * 0.5
        nonzero_mask = abs_sign_sum > 0
        I_sign[nonzero_mask] = 0.5 * (
            1 + sign_sum[nonzero_mask] / abs_sign_sum[nonzero_mask]
        )

        sign_median = np.median(I_sign)
        sign_std = np.std(I_sign)
        sign_scores = 1 / (1 + torch.abs((I_sign - sign_median) / sign_std))
        sign_scores = sign_scores / sign_scores.sum()
