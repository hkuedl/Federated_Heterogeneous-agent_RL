...
from Algorithm.sddpg import SDDPG
from Algorithm.sddpg_master import mSDDPG
...
def global_model_training(variant):
    ...
    for i in range(variant['num_agent']):
        ...
        agent[key] = SDDPG(obs_dim, action_dim, variant, device, variant['seed'][i])
        master[key] = mSDDPG(obs_dim, action_dim, variant, device, variant['seed'][i])
...
def know_transfer(variant):
    ...
    for i in range(variant['num_agent']):
        ...
        obs_dim = env[key].get_obs_dim()
        action_dim = env[key].get_action_dim()
        agent[key] = SDDPG(obs_dim, action_dim, variant, device, variant['seed'][key-1])
        master[key] = mSDDPG(obs_dim, action_dim, variant, device, variant['seed'][key-1])
